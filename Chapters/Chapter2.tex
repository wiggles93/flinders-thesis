...
Cryptography has been investgated by multiple sources as a solution to many aspects of the GNSS spoofing problem. For constellations that have been operational for many
...
similar to that used to secure internet communicatrions. This has manifested in the OS-NMA (Open Service Navigation Message Authentication). This allows modern high
performance receviers to be able to enable the cryptographic solution, while still maintaining backwards compatability for less capable receivers or for those that do
not require the extra security. \todo{add references} \\ 
While this thesis will be concetrating on spoofing and countermeasures of the GPS satellite constellation, it is worth noting that this kind of implementation will be
...
When testing, both covert and overt spoofing methods were used, distuinguished by whether or not the spoofer made an attempt to avoid detection. For the most part there
...
attack. Field testing showed that the spoof attempt caused unrecoverable navigation errors which resulted in the UAV crashing. 
...
\todo{This is a really good resource for how GNSS systems work. Use this extensively in the introduction}
...
strengths of approximatelty $-153dBW$ to $-160dbW$, this ratio is easy to achieve. \todo{how many watts would be required to get the ratio 1.1?} It was noted that the
...
\todo{Fact check the claims of the paper} 
...
existing systems. Four simualtions were tested from a Neyman-Pearson perspective \todo{add in a info about neyman pearson in electrical eng} (1) two receivers with known
...
solutions used in the aviation industry dating back to 1947. From 1973 onwards GPS has been in use, and only expanded to include other GNSS consterllations. Other types
of navigation solutions are out of the scope of this document.
\citeauthor{RN34} seperated the interferences into man-made and channel-based, with the man-made being further seperated into intentional and unintentional. Examples of
intentional interference were jamming, spoofing and meconing, with unintentional interference coming from adjacent channels and co-channels. Factors like atmosphereic
conditions or space weather, multipath communicaitons, doppler effects and scattering all form unintentional interference. A detailed summary of mitigation strategies
based on the attack/interference type was provided. By grouping all of the resources together this paper was able to produce a single repository where defennces against
...
attempt to include spoofing countermeasures within the message structure from the satellite. Software prfiling was performed with performance comparisons undertaken
...
platforms and utelising more powerful hardware to close to gap with more powerful PC machines.
...
environments. From a high level the assimilator will contain an RF front end with multi system receiver and navigation and timeing fusion module,  
...
performs all necessary signal processing to output an L1 C/A signal that the target GPS receiver will be able to interperet with accurate PVT data. The assimilator can be
...
By having a seperate inline component, especially one based off of SDR technologies, upgrades can be applied to any GPS receiver without needing to make modifications to
...
frequency single constellation reciver can be upgraded to be a multi-frequency, multi-constellation receiver.

Future work on the assimilator include miniturising the design in order to fit inside of an antenna such that a user would be able to include all of the benefits of using
the assimilator by only needing to chnage the antenna used. 

\medskip

\textbf{\emph{\citetitle{RN20}} by \citeauthor{RN20}} \\
In \citedate{RN20} \citeauthor{RN20} \cite{RN20} addressed the issue of GNSS signal aquisition when exposed to interference by using an array of antennae.
...
which was able to mitigate temporarily uncorrellated point source interferences. \todo{get help understanding this paper}

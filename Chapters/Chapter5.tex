...
successful in the sense that the receiver was able to interperate a location. The calcualted locationw had a high error and there was clearly not a consistent lock. This
experiment should have been redone ensuring that there was no interefecence in the Faraday cage causing the lock to be lost. 
...
were more SVIDs (or PRN codes) than expected, with more SVIDs recoreded than there are GPS satellites in operation. 
...
It can be see that there is consistent signals for the first 50 seconds. For the time pereiod between 70 and
...
cause of the spikes would be found. From Figure \ref{fig:MABdynamicCoord} it can be seen that initially there was high amounts of error while getting the inital fix.
Though when the movement began there was good tracking. This is evident in Figure \ref{fig:MABdynamicPosition} where the reference path and recevied path were overlayed.
Overlaying onto a satellite image yielded \ref{fig:MABdynamicSatelliteImage} which shows how well the spoof attack was able to track the refence path. This produced
results that would be expeccted from authentic GPS signals, with the exception being the inital part of the experiment. 
...
penetrating the Faraday cage. It is unlikely that the GPS-SDR-sim program includes augmenetation systems when geneating spoofed signals. The orbit of the QZSS satellites
does include Australia so seems most likey an issue where an unterminated cable is allowing the signal to bleed into the cage.
